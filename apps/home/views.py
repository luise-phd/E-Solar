...
spark = (SparkSession
            .builder
            .appName('SIEVARLAR-pyspark-read-and-write-from-hive')
            .enableHiveSupport()
            .getOrCreate())

@login_required(login_url="/e-solar/login/")
def index(request):
...
def procesar_imgs(request):
    ...
    if(lat < -1.64 or lat > 12.41):
        band = -1
    else:
        if(lon > -66.8 or lon < -78.8):
            band = -1
        else:
            t = threading.Thread(target = realizar_prediccion, args = (request, nom, year, lat, lon))
            t.start()
...
def realizar_prediccion(request, nom, year, lat, lon):
    # El siguiente código se movió de la funcion procesar imágenes

    files = os.listdir('imgs/' + year + '/nc/')
    images = []
    for elem in files:
        images.append(elem)
    ...
    spark.catalog.setCurrentDatabase("sandbox")
    df_ccal= spark.sql("SELECT * FROM coef_calibracion").toPandas()
    df_fcorr = spark.sql("SELECT * FROM factor_correccion").toPandas()
    ...
    x = 0
    for i in images:
        goes_vis = rasterio.open('imgs/' + year + '/nc/' + i)
        # print(goes_vis)
        ...
        ccal = df_ccal[(df_ccal['satelite'] == 'GOES-13')]
        
        # La siguiente instrucción genera un error luego de un tiempo sin
        # autenticar el cluster: maprlogin password
        k = float(ccal['k'])
        ...
        # ann = l[3][:4]
        # mes = l[3][4:6]
        fcorr = df_fcorr[(df_fcorr['anno'] == str(ann)) & (df_fcorr['mes'] == str(mes))]
        ...
        print('Reflectancia del pixel:', Rp)
    ...
    # El código anterior se movió de la funcion procesar imágenes
